		...
		if (isset($data['product_option'])) {
			...
			if (isset($data['product_store'])) {
				foreach ($data['product_store'] as $store_id) {	
					...
					if ($NotifyWhenAvailable['notifywhenavailable']['Enabled']=='yes') {
						$this->load->model('extension/module/notifywhenavailable');
						
						$users = $this->model_extension_module_notifywhenavailable->getAllSubscribedCustomers($store_id); // Get all subscribed users from DB
						...
						foreach($users as $user) { 
							...
							if($userCount == 0)	{ // if we don't have to check options quantities
								if ($data['notifywhenavailable']=="yes" && (int)$data['quantity']>0) {	
									$this->model_extension_module_notifywhenavailable->sendEmailWhenAvailable($product_id,$store_id);
								} 
							} else if(!empty($user['selected_options']) && is_array($user['selected_options'])) { // check options quantitites
