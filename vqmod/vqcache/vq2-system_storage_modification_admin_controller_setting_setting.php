...
public function cartmotion_lwi() {
				$mod = 'cartmotion_lwi';
				$heading = 'Lazy WebP Images [by CARTmotion]';
				$this->load->model('setting/setting');
				$set = $this->model_setting_setting->getCARTmotionLWI();
				$tpl = str_replace('<title>', '<title>' . $heading, $this->load->controller('common/header'));
				$tpl .= $this->load->controller('common/column_left');
				$tpl .= '<div id="content">';
				$tpl .= '<div class="page-header">';
				$tpl .= '<div class="container-fluid">';
				if ($set[$mod . '_license']) {
					$tpl .= '<div class="pull-right">';
					$tpl .= '<button type="submit" form="form" data-toggle="tooltip" title="Save" class="btn btn-primary"><i class="fa fa-save"></i></button>';
					$tpl .= '</div>';
				}
				$tpl .= '<h1>' . $heading . '</h1>';
				$tpl .= '<ul class="breadcrumb">';
				$tpl .= '<li><a href="' . $this->url->link('common/dashboard', 'token=' . $this->session->data['token'], true) . '">' . $this->language->get('text_home') . '</a></li>';
				$tpl .= '<li><a href="' . $this->url->link('setting/setting/' . $mod, 'token=' . $this->session->data['token'], true) . '">' . $heading . '</a></li>';
				$tpl .= '</ul>';
				$tpl .= '</div>';
				$tpl .= '</div>';
				if ($this->request->server['REQUEST_METHOD'] == 'POST') {
					if (isset($this->request->post[$mod . '_license'])) {
						$this->model_setting_setting->editCARTmotionLWI($this->request->post);
					}
					elseif (isset($this->request->post['email'])) {
						$this->model_setting_setting->addCARTmotionLWI($this->request->post);
					}
					else {
						$this->session->data['warning'] = 'Warning: Invalid request!';
					}
				}
				elseif (isset($this->request->get['remove'])) {
					$this->model_setting_setting->removeCARTmotionLWI();
				}
				$tpl .= '<div class="container-fluid">';
				if (isset($this->session->data['success'])) {
					$tpl .= '<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' . $this->session->data['success'];
					$tpl .= '<button type="button" class="close" data-dismiss="alert">&times;</button>';
					$tpl .= '</div>';
					unset($this->session->data['success']);
				}
				if (isset($this->session->data['warning'])) {
					$tpl .= '<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' . $this->session->data['warning'];
					$tpl .= '<button type="button" class="close" data-dismiss="alert">&times;</button>';
					$tpl .= '</div>';
				...
				if ($set[$mod . '_license']) {
					...
					$tpl .= '<h3 class="panel-title"><i class="fa fa-flash"></i> ' . $heading . '</h3>';
					$tpl .= '</div>';
					$tpl .= '<div class="panel-body">';
					$tpl .= '<form action="' . $this->url->link('setting/setting/' . $mod, 'token=' . $this->session->data['token'], true) . '" method="post" id="form" class="form-horizontal" enctype="multipart/form-data">';
					$tpl .= '<ul class="nav nav-tabs">';
					$tpl .= '<li class="active"><a href="#tab-store" data-toggle="tab"><i class="fa fa-shopping-cart"></i> Store</a></li>';
					$tpl .= '<li><a href="#tab-support" data-toggle="tab"><i class="fa fa-life-saver"></i> Help</a></li>';
					$tpl .= '<li><a href="#tab-tools" data-toggle="tab"><i class="fa fa-wrench"></i> Tools</a></li>';
					$tpl .= '<li><a href="#tab-license" data-toggle="tab"><i class="fa fa-lock"></i> License</a></li>';
					$tpl .= '</ul>';
					$tpl .= '<input type="hidden" name="' . $mod . '_license" value="' . $set[$mod . '_license'] . '">';
					$tpl .= '<div class="tab-content">';
					$tpl .= '<div class="tab-pane active" id="tab-store">';
					$tpl .= '<h2 class="well">Store Settings</h2>';
					$tpl .= '<fieldset>';
					$tpl .= '<legend>WebP Format</legend>';
					$tpl .= '<div class="form-group">';
					$tpl .= '<label class="col-sm-3 control-label"><span data-toggle="tooltip" title="You can enable the use of WebP images for supported browsers.">WebP Images</span></label>';
					$tpl .= '<div class="col-sm-9">';
					$gd = gd_info();
					if ($gd['WebP Support']) {
						$tpl .= '<div class="btn-group">';
						if ($set[$mod . '_webp_status']) {
							$tpl .= '<label class="btn btn-toggle btn-success">Enabled';
							$tpl .= '<input type="radio" name="' . $mod . '_webp_status" value="1" style="display:none;" checked="checked" />';
							$tpl .= '</label>';
							$tpl .= '<label class="btn btn-toggle btn-default">Disabled';
							$tpl .= '<input type="radio" name="' . $mod . '_webp_status" value="0" style="display:none;" />';
							$tpl .= '</label>';
						}
						else {
							$tpl .= '<label class="btn btn-toggle btn-default">Enabled';
							$tpl .= '<input type="radio" name="' . $mod . '_webp_status" value="1" style="display:none;" />';
							$tpl .= '</label>';
							$tpl .= '<label class="btn btn-toggle btn-danger">Disabled';
							$tpl .= '<input type="radio" name="' . $mod . '_webp_status" value="0" style="display:none;" checked="checked" />';
							$tpl .= '</label>';
						}
						$tpl .= '</div>';
					}
					else {
						$tpl .= '<button type="button" class="btn btn-danger">Your server does not have GD WebP support</button>';
						$tpl .= '<input type="hidden" name="' . $mod . '_webp_status" value="0" />';
					}
					$tpl .= '</div>';
					$tpl .= '</div>';
					$tpl .= '<div class="form-group">';
					$tpl .= '<label class="col-sm-3 control-label"><span data-toggle="tooltip" title="You can choose the quality for WebP images.">WebP Quality (%)</span></label>';
					$tpl .= '<div class="col-sm-3">';
					$tpl .= '<div class="input-group">';
					$tpl .= '<span class="input-group-btn"><button class="btn btn-default btn-add" type="button"><i class="fa fa-plus"></i></button></span>';
					$tpl .= '<input type="text" name="' . $mod . '_webp_quality" value="' . $set[$mod . '_webp_quality'] . '" min="1" max="100" placeholder="eg: 85" class="form-control text-center" spellcheck="false" autocomplete="off">';
					$tpl .= '<span class="input-group-btn"><button class="btn btn-default btn-subtract" type="button"><i class="fa fa-minus"></i></button></span>';
					$tpl .= '</div>';
					$tpl .= '</div>';
					$tpl .= '</div>';
					$tpl .= '</fieldset>';
					$tpl .= '<br>';
					$tpl .= '<fieldset>';
					$tpl .= '<legend>Lazy Loading</legend>';
					$tpl .= '<div class="form-group">';
					$tpl .= '<label class="col-sm-3 control-label"><span data-toggle="tooltip" title="You can enable lazy loading for images outside the browser viewport.">Lazy Images</span></label>';
					$tpl .= '<div class="col-sm-9">';
					$tpl .= '<div class="btn-group">';
					if ($set[$mod . '_lazy_status']) {
						$tpl .= '<label class="btn btn-toggle btn-success">Enabled';
						$tpl .= '<input type="radio" name="' . $mod . '_lazy_status" value="1" style="display:none;" checked="checked" />';
						$tpl .= '</label>';
						$tpl .= '<label class="btn btn-toggle btn-default">Disabled';
						$tpl .= '<input type="radio" name="' . $mod . '_lazy_status" value="0" style="display:none;" />';
						$tpl .= '</label>';
					}
					else {
						$tpl .= '<label class="btn btn-toggle btn-default">Enabled';
						$tpl .= '<input type="radio" name="' . $mod . '_lazy_status" value="1" style="display:none;" />';
						$tpl .= '</label>';
						$tpl .= '<label class="btn btn-toggle btn-danger">Disabled';
						$tpl .= '<input type="radio" name="' . $mod . '_lazy_status" value="0" style="display:none;" checked="checked" />';
						$tpl .= '</label>';
					}
					$tpl .= '</div>';
					$tpl .= '</div>';
					...
					$tpl .= '<div class="form-group">';
					$tpl .= '<label class="col-sm-3 control-label"><span data-toggle="tooltip" title="You can choose the placeholder for images outside the browser viewport.">Lazy Placeholder</span></label>';
					$tpl .= '<div class="col-sm-9">';
					$tpl .= '<a href="" id="thumb-lazy-placeholder" data-toggle="image" class="img-thumbnail"><img src="' . $this->model_tool_image->resize($set[$mod . '_lazy_placeholder'], 100, 100) . '" alt="" title="" data-placeholder="' . $this->model_tool_image->resize('no_image.png', 100, 100) . '" /></a>';
					$tpl .= '<input type="hidden" name="' . $mod . '_lazy_placeholder" value="' . $set[$mod . '_lazy_placeholder'] . '" id="input-lazy-placeholder" />';
					$tpl .= '</div>';
					$tpl .= '</div>';
					$tpl .= '</fieldset>';
					$tpl .= '</div>';
					$tpl .= '<div class="tab-pane" id="tab-support">';
					$tpl .= '<h2 class="well">Help & Support</h2>';
					$tpl .= '<p>I hope you are enjoying this extension.</p>';
					$tpl .= '<p>My name is Raul Maro&#x219;an, I am a web developer based in <a href="https://en.wikipedia.org/wiki/Transylvania" data-toggle="tooltip" title="Yes, that Transylvania!" target="blank_">Transylvania</a>.</p>';
					$tpl .= '<p>I optimize e-commerce websites for Robots and Humans. And <a href="https://cartmotion.com/blog/" data-toggle="tooltip" title="CARTmotion E-commerce Blog" target="blank_">blog</a> about it.</p>';
					$tpl .= '<p>I am specialized in:</p>';
					$tpl .= '<ul style="list-style:none;">';
					$tpl .= '<li><i class="fa fa-check"></i> <em>Website Speed</em></li>';
					$tpl .= '<li><i class="fa fa-check"></i> <em>Technical SEO</em></li>';
					$tpl .= '<li><i class="fa fa-check"></i> <em>Website Structure & Content</em></li>';
					$tpl .= '</ul>';
					$tpl .= '<p>You can contact me by email at <a href="mailto:raul@cartmotion.com" data-toggle="tooltip" title="Email Me">raul@cartmotion.com</a> or using the <a href="https://cartmotion.com/contact/" data-toggle="tooltip" title="Contact Form" target="blank_">online form</a>.</p>';
					$tpl .= '<p>Let me know if you need any help!</p>';
					$tpl .= '</div>';
					$tpl .= '<div class="tab-pane" id="tab-tools">';
					$tpl .= '<h2 class="well">Tools & Maintenance</h2>';
					$tpl .= '<fieldset>';
					$tpl .= '<legend>Extension</legend>';
					$tpl .= '<div class="form-group">';
					$tpl .= '<label class="col-sm-3 control-label"><span data-toggle="tooltip" title="You can temporarily disable the extension for maintenance.">Status</span></label>';
					$tpl .= '<div class="col-sm-9">';
					$tpl .= '<div class="btn-group">';
					if ($set[$mod . '_status']) {
						$tpl .= '<label class="btn btn-toggle btn-success">Enabled';
						$tpl .= '<input type="radio" name="' . $mod . '_status" value="1" style="display:none;" checked="checked" />';
						$tpl .= '</label>';
						$tpl .= '<label class="btn btn-toggle btn-default">Disabled';
						$tpl .= '<input type="radio" name="' . $mod . '_status" value="0" style="display:none;" />';
						$tpl .= '</label>';
					}
					else {
						$tpl .= '<label class="btn btn-toggle btn-default">Enabled';
						$tpl .= '<input type="radio" name="' . $mod . '_status" value="1" style="display:none;" />';
						$tpl .= '</label>';
						$tpl .= '<label class="btn btn-toggle btn-danger">Disabled';
						$tpl .= '<input type="radio" name="' . $mod . '_status" value="0" style="display:none;" checked="checked" />';
						$tpl .= '</label>';
					}
					$tpl .= '</div>';
					$tpl .= '</div>';
					$tpl .= '</div>';
					$tpl .= '<div class="form-group">';
					$tpl .= '<label class="col-sm-3 control-label"><span data-toggle="tooltip" title="You can revoke the current license and disable the extension.">Uninstall</span></label>';
					$tpl .= '<div class="col-sm-9">';
					$tpl .= '<a href="' . $this->url->link('setting/setting/' . $mod, '&remove&token=' . $this->session->data['token'], true) . '" class="btn btn-danger">Remove License</a>';
					$tpl .= '</div>';
					$tpl .= '</div>';
					$tpl .= '</fieldset>';
					$tpl .= '</div>';
					$tpl .= '<div class="tab-pane" id="tab-license">';
					$tpl .= '<h2 class="well">License Information</h2>';
					$tpl .= '<p><i class="fa fa-check"></i> This license grants you permission to install and use "Lazy WebP Images" for an unlimited period.</p>';
					$tpl .= '<p><i class="fa fa-check"></i> Licensing or sublicensing of "Lazy WebP Images" in its original or modified form is not permitted.</p>';
					$tpl .= '<p><i class="fa fa-check"></i> Free publication or reselling of "Lazy WebP Images" in its original or modified form is not permitted.</p>';
					$tpl .= '<p><i class="fa fa-check"></i> Integration of "Lazy WebP Images" with your own or other third-party product is not permitted.</p>';
					$tpl .= '<p><i class="fa fa-check"></i> Using parts of the source code from "Lazy WebP Images" in your own or other third-party product is not permitted.</p>';
					$tpl .= '<p><i class="fa fa-check"></i> Copyrights of "Lazy WebP Images" are reserved exclusively to Raul Maro&#x219;an.</p>';
					$tpl .= '<p><i class="fa fa-check"></i> Copyright and license information removal is not permitted.</p>';
					$tpl .= '<br><br><small>"Lazy WebP Images" is available for download only on www.opencart.com<br>If you have downloaded or purchased "Lazy WebP Images" from another website please send an email to office@cartmotion.com regarding copyright infringement.</small>';
					$tpl .= '<br><br><small>Developed at <a href="https://cartmotion.com" data-toggle="tooltip" title="The E-commerce Incubator" target="blank_">CARTmotion</a> by Raul Maro&#x219;an &copy; 2013-' . date('Y') . '</small>';
					$tpl .= '</div>';
					$tpl .= '</div>';
					$tpl .= '</form>';
					$tpl .= '</div>';
					...
					$tpl .= '$("body").on("click", ".btn-toggle", function() {';
					$tpl .= '$(this).parent().find(".btn-toggle").attr("class", "btn btn-toggle btn-default");';
					$tpl .= 'if ($(this).is(":last-child")) {';
					$tpl .= '$(this).addClass("btn-danger");';
					$tpl .= '} else {';
					$tpl .= '$(this).addClass("btn-success");';
					$tpl .= '}';
					$tpl .= '});';
					$tpl .= '$("body").on("click", ".btn-add", function() {';
					$tpl .= 'var field = $(this).parent().siblings("input");';
					$tpl .= 'var value = field.attr("min");';
					$tpl .= 'if (field.val() != false && !isNaN(field.val())) {';
					$tpl .= 'if (parseInt(field.val()) < parseInt(field.attr("max"))) {';
					$tpl .= 'var value = parseInt(field.val()) + 1;';
					$tpl .= '}';
					$tpl .= '}';
					$tpl .= 'field.val(value);';
					$tpl .= '});';
					$tpl .= '$("body").on("click", ".btn-subtract", function() {';
					$tpl .= 'var field = $(this).parent().siblings("input");';
					$tpl .= 'var value = field.attr("max");';
					$tpl .= 'if (field.val() != false && !isNaN(field.val())) {';
					$tpl .= 'if (parseInt(field.val()) > parseInt(field.attr("min"))) {';
					$tpl .= 'var value = parseInt(field.val()) - 1;';
					$tpl .= '}';
					$tpl .= '}';
					$tpl .= 'field.val(value);';
					...
					$tpl .= '<br><br><br>';
					$tpl .= '<div class="col-sm-offset-3 col-sm-6">';
					$tpl .= '<div class="panel panel-default">';
					$tpl .= '<div class="panel-heading">';
					$tpl .= '<h3 class="panel-title"><i class="fa fa-lock"></i> Unlock Extension</h3>';
					$tpl .= '</div>';
					$tpl .= '<div class="panel-body">';
					$tpl .= '<form action="' . $this->url->link('setting/setting/' . $mod, 'token=' . $this->session->data['token'], true) . '" method="post" class="form-horizontal">';
					$tpl .= '<fieldset>';
					$tpl .= '<legend>License Information</legend>';
					$tpl .= '<div class="form-group">';
					$tpl .= '<label class="col-sm-2 control-label">Domain</label>';
					$tpl .= '<div class="col-sm-10">';
					$tpl .= '<p class="form-control-static">' . (isset($this->request->server['HTTP_HOST']) ? $this->request->server['HTTP_HOST'] : $this->request->server['SERVER_NAME']) . '</p>';
					$tpl .= '</div>';
					$tpl .= '</div>';
					$tpl .= '<div class="form-group">';
					$tpl .= '<label class="col-sm-2 control-label">Name</label>';
					$tpl .= '<div class="col-sm-10">';
					$tpl .= '<input type="text" name="name" value="" placeholder="Name" class="form-control">';
					$tpl .= '</div>';
					$tpl .= '</div>';
					$tpl .= '<div class="form-group">';
					$tpl .= '<label class="col-sm-2 control-label">Email</label>';
					$tpl .= '<div class="col-sm-10">';
					$tpl .= '<input type="text" name="email" value="" placeholder="Email Address" class="form-control">';
					$tpl .= '</div>';
					$tpl .= '</div>';
					$tpl .= '<div class="form-group">';
					$tpl .= '<div class="col-sm-12">';
					$tpl .= '<button type="submit" class="btn btn-primary pull-right"><i class="fa fa-key"></i> Activate License</button>';
					$tpl .= '</div>';
					$tpl .= '</div>';
					$tpl .= '</fieldset>';
					$tpl .= '</form>';
				...
				$tpl .= '</div>';
				$tpl .= '</div>';
				$tpl .= $this->load->controller('common/footer');
				$tpl .= '</div>';
