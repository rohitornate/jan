...
class ModelToolExportImport extends Model {
	...
	protected function storeProductIntoDatabase( &$product, &$languages, &$product_fields, $exist_table_product_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		...
		$video_url = $this->db->escape($product['video_url']);
		...
		$sql .= "`location`,`no_follow`,`alt`,`stock_status_id`,`model`,`manufacturer_id`,`image`,`video_url`,`shipping`,`price`,`points`,`date_added`,`date_modified`,`date_available`,`weight`,`weight_class_id`,`status`,";
		...
		$sql .= in_array('isbn',$product_fields) ? "'$isbn'," : "";
		$sql .= in_array('mpn',$product_fields) ? "'$mpn'," : "";
		$sql .= "'$location','$no_follow','$alt',$stock_status_id,'$model',$manufacturer_id,'$image','$video_url',$shipping,$price,$points,";
		$sql .= ($date_added=='NOW()') ? "$date_added," : "'$date_added',";
		$sql .= ($date_modified=='NOW()') ? "$date_modified," : "'$date_modified',";
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$video_url = $this->getCell($data,$i,$j++);
	...
	protected function storeAdditionalImageIntoDatabase( &$image, &$old_product_image_ids, $exist_sort_order=true ) {
		...
		if (isset($old_product_image_ids[$product_id][$image_name])) {
			...
			if ($exist_sort_order) {
				$sql  = "INSERT INTO `".DB_PREFIX."product_image` (`product_image_id`,`product_id`,`image`,`sort_order`,`image_alt` ) VALUES "; 
				$sql .= "($product_image_id,$product_id,'".$this->db->escape($image_name)."',$sort_order,'".$this->db->escape($image_alt)."')";
			} else {
				$sql  = "INSERT INTO `".DB_PREFIX."product_image` (`product_image_id`,`product_id`,`image`,`image_alt` ) VALUES "; 
				$sql .= "($product_image_id,$product_id,'".$this->db->escape($image_name)."','".$this->db->escape($image_alt)."')";
			}
			$this->db->query($sql);
			unset($old_product_image_ids[$product_id][$image_name]);
		} else {
			if ($exist_sort_order) {
				$sql  = "INSERT INTO `".DB_PREFIX."product_image` (`product_id`,`image`,`sort_order`,`image_alt`  ) VALUES "; 
				$sql .= "($product_id,'".$this->db->escape($image_name)."',$sort_order,'".$this->db->escape($image_alt)."')";
			} else {
				$sql  = "INSERT INTO `".DB_PREFIX."product_image` (`product_id`,`image`,,`image_alt`  ) VALUES "; 
				$sql .= "($product_id,'".$this->db->escape($image_name)."','".$this->db->escape($image_alt)."')";
			}
			$this->db->query($sql);
		}
	}
	...
	protected function uploadAdditionalImages( &$reader, $incremental, &$available_product_ids ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$image_alt = $this->getCell($data,$i,$j++,'');
			$image = array();
	...
	protected function validateProducts( &$reader ) {
		...
		$expected_heading = array_merge( $expected_heading, array( "location","no_follow","alt", "quantity", "model", "manufacturer", "image_name", "video_url","shipping", "price", "points", "date_added", "date_modified", "date_available", "weight", "weight_unit", "length", "width", "height", "length_unit", "status", "tax_class_id", "seo_keyword", "description") );
		if ($exist_meta_title) {
...
//		}
	...
	protected function populateProductsWorksheet( &$worksheet, &$languages, $default_language_id, &$price_format, &$box_format, &$weight_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null) {
		...
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('video_url'),12)+1);
	...
	protected function getAdditionalImages( $min_id=null, $max_id=null, $exist_sort_order=true  ) {
		if ($exist_sort_order) {
			$sql  = "SELECT product_id, image, sort_order,image_alt ";
		} else {
			$sql  = "SELECT product_id, image,image_alt ";
		}
		...
		if ($exist_sort_order) {
			$sql .= "ORDER BY product_id, sort_order, image,image_alt;";
		} else {
			$sql .= "ORDER BY product_id, image,image_alt;";
		}
		$result = $this->db->query( $sql );
		return $result->rows;
	}
	...
	protected function populateAdditionalImagesWorksheet( &$worksheet, &$box_format, &$text_format, $min_id=null, $max_id=null) {
		...
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(max(strlen('alt'),30)+1);
