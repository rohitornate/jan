            ...
            <tbody>
              <tr>
                ...
                <td class="text-left" style="width:30%;vertical-align:top;">
                  ...
                  <div class="input-group"><span class="input-group-addon"><img src="language/<?php echo $language['code']; ?>/<?php echo $language['code']; ?>.png" title="<?php echo $language['name']; ?>" /></span><input type="text" name="paycharge[0][description][<?php echo $language['language_id']; ?>][name]" value="<?php echo isset($paycharge['description'][$language['language_id']]) ? $paycharge['description'][$language['language_id']]['name'] : ''; ?>" class="form-control" /></div>
                  <?php } ?>
                </td>
			  ...
			  <tr>
                <td class="text-left" style="width:30%;vertical-align:top;">
                  <select name="paycharge[1][payment_method]" class="form-control">
                    <?php foreach ($payments as $payment) { ?>
                    <?php if ($payment['code'] == $paycharge['payment_method']) { ?>
                    <option value="<?php echo $payment['code']; ?>" selected="selected"><?php echo $payment['name']; ?></option>
                    <?php } else { ?>
                ...
                <td class="text-left" style="width:30%;vertical-align:top;">
                  <div class="input-group">
                    <span class="input-group-addon" style="min-width:40px;"><?php echo $entry_percentage[0]; ?></span>
                    <input type="text" name="paycharge[1][valuep]" value="<?php echo $paycharge['valuep']; ?>" placeholder="<?php echo $entry_percentage[1]; ?>" class="form-control" />
                  </div>
                </td>
                <td class="text-left" style="width:30%;vertical-align:top;">
                  <?php foreach ($languages as $language) { ?>
                  <div class="input-group"><span class="input-group-addon"><img src="language/<?php echo $language['code']; ?>/<?php echo $language['code']; ?>.png" title="<?php echo $language['name']; ?>" /></span><input type="text" name="paycharge[1][description][<?php echo $language['language_id']; ?>][name]" value="<?php echo isset($paycharge['description'][$language['language_id']]) ? $paycharge['description'][$language['language_id']]['name'] : ''; ?>" class="form-control" /></div>
                  <?php } ?>
                </td>
                <td class="text-left"><button type="button" onclick="upgrade();" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>
              </tr>
...
function upgrade() {
	var upPayCharge = confirm('You are running the Demontration version of PayCharge.\n\nPayCharge is available in Plus and Pro version.\n\nUpgrade Now Your PayCharge to get all features!\n\n- More rules\n- Fixed and Percentage values\n- Tax Classes\n- Customer Groups\n- More options.\n\n');

	if (upPayCharge) {
		window.location.href = 'http://www.opencart.com/index.php?route=marketplace/extension&filter_member=fabiom7';
	}
}
</script>
<?php echo $footer; ?>