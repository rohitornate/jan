$(document).ready(function(){
 	...
 	currentList = "index.php?route=extension/module/giftteaser/giftList&token=" + token + "&store_id=" + store;
    ...
    $('.btn.btn-primary.save-changes').trigger('click');
        window.location = "index.php?route=extension/module/giftteaser&store_id=" + $('select[name="selectStore"]').val() + "&token=" + token;
    });
...
function removeGift(ob, refreshList) {
	...
	bootbox.confirm('Are you sure you want to delete this gift?', function(result){
		if(result){ 
			row.remove();
			$.ajax({
				url: "index.php?route=extension/module/giftteaser/removeGift&token=" + token,
...
function editItem(gift_id, item_id, item_name) {
 	link = './index.php?route=extension/module/giftteaser/giftForm&gift_id=' + gift_id + '&item_id=' + item_id + '&item_name=' + encodeURIComponent(item_name) + '&token=' + token;
...
function saveItem(ob) {
	...
	$.ajax({   
		url: 'index.php?route=extension/module/giftteaser/giftForm&token=' + token,
		type: 'post',
		dataType:'html', 
		data: giftData, 
		success: function(data) {
...
function autocompleteProduct(ob, action) { 
	ob.autocomplete({
		...
		source: function (request, response) {
			$.ajax({
				url: 'index.php?route=extension/module/giftteaser/autocompleteProduct&token=' + getURLVar('token'),
				dataType: 'json',
				type: 'POST',
...
function autocompleteCategory(ob, action) {
	ob.autocomplete({
		...
		source: function (request, response) {
			$.ajax({
				url: 'index.php?route=extension/module/giftteaser/autocompleteCategory&token=' + getURLVar('token')  + '&store_id=' + store,
				dataType: 'json',
				type: 'POST',
				data: {filter_name: request, store_id: store},
				success: function (json) { 
