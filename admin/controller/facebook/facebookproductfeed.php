...
class ControllerFacebookFacebookProductFeed extends Controller {
  ...
  const FEED_FOLDER_NOT_WRITABLE_EXCEPTION_MESSAGE =
    'We need the ' . DIR_MODIFICATION . ' folder on your server to be writable to allow for initial product upload. Please enable the folder to be writable and try again.';
  ...
  public function getSampleProductFeed() {
    ...
    $feed_items = array_map(function($product) {
      return $data = $this->facebookproductfeedformatter->getProductData(
        $product);
    }, $products);
  ...
  public function syncAllProductsUsingFeed() {
    ...
    if (!$facebook_catalog_id || !$facebook_page_token) {
      $this->logError(
        self::NO_CATALOG_ID_ACCESS_TOKEN_ERROR_MESSAGE . $operation,
        $error_data,
        self::INITIAL_PRODUCT_SYNC_EXCEPTION_MESSAGE);
    }
    ...
    if (!$productFeedFullFilename) {
      $this->logError(
        ...
        $error_data,
        self::FEED_FOLDER_NOT_WRITABLE_EXCEPTION_MESSAGE);
    }
  ...
  public function getInitialProductSyncStatus() {
    ...
    $this->validateFAEAndProductSync(
      $operation,
      $error_data);
  ...
  public function isWritableProductFeedFolderAvailable() {
    if ($this->getWritableProductFeedFolder()) {
      ...
      throw new Exception(self::FEED_FOLDER_NOT_WRITABLE_EXCEPTION_MESSAGE);
    }
  }
}
