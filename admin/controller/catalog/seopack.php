...
class ControllerCatalogSeoPack extends Controller { 
	...
	public function seourls($r = 0) {
		...
		if (!$r) {$this->response->redirect($this->url->link('catalog/seopack', 'token=' . $this->session->data['token'], 'SSL'));}
   ...
   function _transliteration_replace($ord, $unknown = '?', $langcode = NULL) {
        ...
        if (!isset($map[$bank][$langcode])) {
            $file = DIR_APPLICATION . 'model/catalog/trans_db/' . sprintf('x%02x', $bank) . '.php';
            if (file_exists($file)) {
                include $file;
