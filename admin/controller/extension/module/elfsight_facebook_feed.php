...
class Deactivate {
    ...
    public function __construct($data) {
        ...
        $this->app_slug = 'facebook-feed';
        $this->app_name = 'Facebook Feed';
    ...
    public function setDeactivateReasons() {
        return require ( (__DIR__) . '/facebook-feed-assets/reasons.php' );
    }
...
class ControllerExtensionModuleElfsightFacebookFeed extends Controller {
	private $error = array();

	public $params = NULL;

    public $ELFSIGHT_EMBED_URL = 'https://apps.elfsight.com/embed/facebook-feed/?utm_source=portals&utm_medium=opencart&utm_campaign=facebook-feed&utm_content=sign-up';
	...
	public function index() {
		$this->load->language('extension/module/elfsight_facebook_feed');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('module_elfsight_facebook_feed', $this->request->post);
		...
		$data['action'] = $this->url->link('extension/module/elfsight_facebook_feed', 'token=' . $this->session->data['token'], true);

		$data['cancel'] = $this->url->link('marketplace/extension', 'token=' . $this->session->data['token'] . '&type=module', true);

		if (isset($this->request->post['module_elfsight_facebook_feed_status'])) {
			$data['module_elfsight_facebook_feed_status'] = $this->request->post['module_elfsight_facebook_feed_status'];
		} else {
			$data['module_elfsight_facebook_feed_status'] = $this->config->get('module_elfsight_facebook_feed_status');
		}

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');

		$this->response->setOutput($this->load->view('extension/module/elfsight_facebook_feed', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/elfsight_facebook_feed')) {
    ...
    public function setDeactivateReasons() {
        return require ( './controller/extension/module/facebook-feed-assets/reasons.php' );
    }
    ...
    public function uninstall() {
        ...
        include './controller/extension/module/facebook-feed-assets/popup.php';
    }
