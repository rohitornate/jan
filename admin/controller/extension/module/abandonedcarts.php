...
class ControllerExtensionModuleAbandonedcarts extends Controller {
	...
	public function sendcustomemail() {
		if (isset($this->request->post) && isset($this->request->post['ABcart_id']) && isset($this->request->post['AB_template_id'])) {	
			...
			/*
			$CartProducts ='<tr>
							<td width="100%" height="5"><hr size="1" width="90%" color="#666666"></td>
						</tr>';
						foreach ($result['cart'] as $product) {  
				$CartProducts .=			'<tr style="display: inline-block;float:left; margin-left: 33px; margin-top: 15px;">
							<td height="200" align="left" style="display:inline-block;float:left; padding-left:0px">
							<a href="#"><img src='.$product["image"].' style="width:158px;margin:0 auto;MARGIN-LEFT:0px"></a>
							</td>
						
						<td>
						<p style="margin-left:38px; display: inline-block; margin-top: 50px;font-family: Ubuntu;font-size: 18px;"> '.$product['name'].'</p>
						</td>
						<td>
						<p style="margin-left:0px; display: inline-block; margin-top: 50px;font-family: Ubuntu;font-size: 18px;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'. $product["price"].'</p>
						</td>
					...
					$CartProducts ='<center>';
									...
									if ($product['image']) {
											$thumb = $this->model_tool_image->resize($product['image'], '400', '400');
										} else {
											$image = false;
										}
						
									$CartProducts .='
									<a href="'.$this->getCatalogURL().'index.php?route=' . $this->modulePath . '/restoreCart&id='.$this->request->post['ABcart_id'].'"	><img src="'. $thumb .'" alt="'. $product['name'] .'" width="300px;"></a>';
										$i++;
									}else {
            ...
            $patterns[8] = '{restore_link}';
			 $patterns[9] = '{recomandateProducts}';
			 $patterns[10] = '{product_price}';
			 $patterns[11] = '{product_name}';
			...
			$replacements[7] = '<a href="'.$this->getCatalogURL().'index.php?route=' . $this->modulePath . '/removeCart&id='.$this->request->post['ABcart_id'].'">'.$this->language->get('text_unsubscribe').'</a>';
          //  $replacements[8] = '<a href="'.$this->getCatalogURL().'index.php?route=' . $this->modulePath . '/restoreCart&id='.$this->request->post['ABcart_id'].'">'.$this->language->get('text_restorecart').'</a>';
		
		
			$replacements[8] ='<a href="'.$this->getCatalogURL().'index.php?route=' . $this->modulePath . '/restoreCart&id='.$this->request->post['ABcart_id'].'" 
													 class="btn1 btn-primary1" style="color: #fff; background-color: #5a015a;
    font-family: Poppins, sans-serif;
    border-color: #5a015a;    display: inline-block;    padding: 6px 12px;    margin-bottom: 0;    font-size: 14px;    font-weight: 400;line-height: 1.42857143;text-align: center;white-space: nowrap;vertical-align: middle;
    ...
    border-radius: 4px;">Checkout
												</a>';
		
		    //  $replacements[8] = '<a href="'.$this->getCatalogURL().'index.php?route=' . $this->modulePath . '/restoreCart&id='.$this->request->post['ABcart_id'].'"  style="text-decoration:none;margin-left:280px;color:#264386;font-size:17px;font-family:Ubuntu;border:1px solid #613f6a;padding:10px 24px;background:#613f6a;color:#fff;border-radius:4px">Checkout</a>';
	
		$replacements[9] = $recomandateProducts;
		$replacements[10] = $product_value;
		$replacements[11] = $product_name;
