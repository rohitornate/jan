...
class ControllerExtensionTotalPaycharge extends Controller {
	...
	public function index() {
		...
		$data['heading_title'] .= ' <small><a href="http://www.opencart.com/index.php?route=extension/extension&filter_username=fabiom7" title="Powered by fabiom7.com" target="_blank">v6.1</a></small>';
		...
		$data['button_cancel'] = $this->language->get('button_cancel');
 		...
 		if (isset($this->error['warning'])) {
   		...
   		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/total/paycharge', 'token=' . $this->session->data['token'], true),
   		);

		$data['action'] = $this->url->link('extension/total/paycharge', 'token=' . $this->session->data['token'], true);
