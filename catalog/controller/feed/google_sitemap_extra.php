...
class ControllerFeedGoogleSitemapExtra extends Controller {
   public function index() {
	  if ($this->config->get('google_sitemap_status')) {
		 ...
		 $fields = array();
		 $query = $this->db->query("SELECT DISTINCT ua.keyword FROM " . DB_PREFIX . "url_alias ua INNER JOIN " . DB_PREFIX . "language l on l.language_id = ua.language_id where l.code = '". $this->db->escape($_GET["language"]) ."';");
		 $fields = $query->rows;
