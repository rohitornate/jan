...
class ControllerAccountReturn extends Controller {
	...
	public function add() {
		...
		if (isset($this->request->post['product_id'])) {
			$data['product'] = $this->request->post['product_id'];
		} elseif (!empty($product_info)) {
		...
		if (isset($this->request->post['bank_ifsc'])) {
			$data['bank_ifsc'] = $this->request->post['bank_ifsc'];
		} else {
			$data['bank_ifsc'] = '';
		}
	...
	//	print_r($_POST);
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$return_id = $this->model_account_return->addCancel($this->request->post);
print_r($return_id);
			// Add to activity log
			if ($this->config->get('config_customer_activity')) {
				$this->load->model('account/activity');
		...
		$data['action'] = $this->url->link('account/return/cancel', '', true);
	...
	protected function validate() {
		if (!$this->request->post['comment']) {
			$this->error['comment'] = $this->language->get('comment');
		}
		return !$this->error;
	}
