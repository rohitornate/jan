...
//            }
		...
		$category_info = $this->model_catalog_category->getCategory($this->_data['currentPath']);
	//print_r($category_info);exit;	
		$this->_data['filter_enable']= $category_info['filter_enable'];
	...
	public function ajaxfilter()
	{
        ...
        if ((bool)$this->_getRequestParam('price', false)) {
			...
			$number = ceil($minMax['max'] / 1000) * 1000;
			
			$min = floor($this->currency->format($minMax['min'], $this->_currency, '', false));
			$max = ceil($this->currency->format($number, $this->_currency, '', false));
            $json['min'] = 0;
            $json['max'] = $max;
		}
