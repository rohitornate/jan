		...
		// Xtensions Best Checkout						
          	if($this->xtensions->isActive()){
            	...
            	$data['custom_js'] = html_entity_decode($modData['js'], ENT_QUOTES, 'UTF-8');
           		$this->response->setOutput($this->load->view('xtensions/checkout/xcheckout.tpl', $data));
           		// Xtensions Best Checkout                		
          	}else {
		...
		if (!$json) {	
			...
			if ($country_info) {
				...
				$pincode=$this->model_account_customer->getDeliveredPincode($this->request->post['postcode']);
						if (!$pincode) {
							$json['error']['postcode']= "WE CAN'T DELIVER TO THIS PINCODE. CAN WE SEND YOUR ORDER TO A DIFFRENT ADDRESS? PLEASE FILL YOUR ADDRESS AGAIN WHERE YOU WANT YOUR ORDER TO BE SHIPPED";
						}
	...
	public function confirm($render = true, &$data = array()) {
		...
		if (isset($_POST) && !empty($_POST)) {
			if ($this->customer->isLogged()) {
			    ...
			    $this->db->query("UPDATE " . DB_PREFIX . "customer SET email = '" . $this->db->escape($_POST['email']) . "', telephone = '" . $_POST['telephone'] . "' WHERE customer_id = '" . $this->customer->getId() . "'");
