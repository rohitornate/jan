...
class ControllerXCheckoutXLogin extends Controller {
	...
	public function index() {
		...
		$this->data['gcaptcha_key'] = $this->config->get('google_captcha_key');
		$this->data['captcha'] = '';
		$this->data['gcaptcha'] = '';
		// Captcha
		if ($this->config->get($this->config->get('config_captcha') . '_status') && in_array('register', (array)$this->config->get('config_captcha_page'))) {
			$this->data['captcha'] = $this->load->controller('xcheckout/x' . $this->config->get('config_captcha'),array('view'=>'register'));
		}
		
		if ($this->config->get($this->config->get('config_captcha') . '_status') && in_array('guest', (array)$this->config->get('config_captcha_page'))) {
			$this->data['gcaptcha'] = $this->load->controller('xcheckout/x' . $this->config->get('config_captcha'),array('view'=>'guest'));
		}
		
		$this->data['forgotten'] = $this->url->link('account/forgotten', '', 'SSL');
		$children = array(
			'xcheckout/xcvc/xoptions',
			'xcheckout/xcvc/xtotals' 
		);
		$this->data += $this->xtensions->getChildren($children);
		$this->template = 'xtensions/checkout/xlogin.tpl';
	...
	public function validate() {
		...
		if ($this->request->post['account'] == 'login') {
			...
			if (!$json && $this->request->post['account'] == 'register') {
				...
				foreach ($custom_fields as $custom_field) {
					if ($custom_field['location'] == 'account') {
						if (($custom_field['type'] == 'text') && !empty($custom_field['validation']) && !filter_var($this->request->post['custom_field'][$custom_field['custom_field_id']], FILTER_VALIDATE_REGEXP, array(
							'options' => array(
						...
						} else if ($custom_field['required'] && (($custom_field['type'] == 'text') || ($custom_field['type'] == 'textarea'))) {
							$field = $this->request->post['custom_field'][$custom_field['custom_field_id']];
							if ((utf8_strlen($field) < $custom_field['minimum']) || (utf8_strlen($field) > $custom_field['maximum'])) {
								$json['error']['custom_field' . $custom_field['custom_field_id']] = $custom_field['error'];
							}
						}
					}
				...
				if ($this->request->post['account'] == 'register') {
					...
					// Captcha
					if ($this->config->get($this->config->get('config_captcha') . '_status') && in_array('register', (array)$this->config->get('config_captcha_page'))) {
						$captcha = $this->load->controller('xcheckout/x' . $this->config->get('config_captcha') . '/validate',array('view'=>'register'));
			...
			if (!$json && $this->request->post['account'] == 'guest') {
				...
				foreach ($custom_fields as $custom_field) {
					if ($custom_field['location'] == 'account') {
						if (($custom_field['type'] == 'text') && !empty($custom_field['validation']) && !filter_var($this->request->post['gcustom_field'][$custom_field['custom_field_id']], FILTER_VALIDATE_REGEXP, array(
							'options' => array(
								'regexp' => $custom_field['validation'] 
							) 
						))) {
							$json['gerror']['custom_field' . $custom_field['custom_field_id']] = sprintf($this->language->get('error_custom_field_validate'), $custom_field['name']);
						}
				...
				// Captcha
				if ($this->config->get($this->config->get('config_captcha') . '_status') && in_array('guest', (array)$this->config->get('config_captcha_page'))) {
					$captcha = $this->load->controller('xcheckout/x' . $this->config->get('config_captcha') . '/validate',array('view'=>'guest'));
				...
				if (!$json) {
					...
					$child = $this->xtensions->getChildren(array(
						'xcheckout/xguest/matter' 
					));
