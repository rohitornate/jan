...
class ControllerXCheckoutXGuest extends Controller {
	...
	public function matter() {
		...
		$this->template = 'xtensions/checkout/xguest.tpl';
	...
	public function addSAddress(){
		...
		if (!$json) {
			...
			foreach ($custom_fields as $custom_field) {
				if($custom_field['location'] == 'address'){
					if (($custom_field['type'] == 'text') && !empty($custom_field['validation']) && !filter_var($this->request->post['scustom_field'][$custom_field['custom_field_id']], FILTER_VALIDATE_REGEXP, array('options' => array('regexp' => $custom_field['validation'])))) {
						$json['error']['custom_field' . $custom_field['custom_field_id']] = sprintf($this->language->get('error_custom_field_validate'), $custom_field['name']);
					}
